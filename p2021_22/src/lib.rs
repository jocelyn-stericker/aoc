...
pub fn part_a(input: &str) -> usize {
    ...
    for line in input.trim().split('\n') {
        ...
        let (mut x_min, mut x_max) = parse_range(dims.next().unwrap());
        let (mut y_min, mut y_max) = parse_range(dims.next().unwrap());
        let (mut z_min, mut z_max) = parse_range(dims.next().unwrap());
...
fn subtract(a: &Cuboid, b: &Cuboid) -> Vec<Cuboid> {
    if let Some(c) = intersection(a, b) {
        let mut v = Vec::new();
        // a -> c, c -> c, c -> a
        for (i, x) in [(a.0 .0, c.0 .0 - 1), (c.0 .0, c.0 .1), (c.0 .1 + 1, a.0 .1)]
            .iter()
            .enumerate()
        {
            for (j, y) in [(a.1 .0, c.1 .0 - 1), (c.1 .0, c.1 .1), (c.1 .1 + 1, a.1 .1)]
                .iter()
                .enumerate()
            {
                for (k, z) in [(a.2 .0, c.2 .0 - 1), (c.2 .0, c.2 .1), (c.2 .1 + 1, a.2 .1)]
                    .iter()
                    .enumerate()
                {
                    if i == 1 && j == 1 && k == 1 {
...
pub fn part_b(input: &str, clamp: bool) -> usize {
    ...
    for (this, this_on) in &commands {
        let mut next_on = Vec::new();
        for cuboid in on {
            for cuboid in subtract(&cuboid, this) {
                next_on.push(cuboid);
            }
        }
        if *this_on {
    ...
    on.into_iter().map(|c| volume(&c)).sum()
}
...
on x=967..23432,y=45373..81175,z=27513..53682\n",
    ...
    #[test]
    fn part_a() {
        assert_eq!(super::part_a(include_str!("input.txt")), 568000);
    }
...
off x=-93533..-4276,y=-16170..68771,z=-104985..-24507\n",
    ...
    fn part_b() {
        assert_eq!(
            super::part_b(include_str!("input.txt"), false),
            1177411289280259
        );
    }
}
